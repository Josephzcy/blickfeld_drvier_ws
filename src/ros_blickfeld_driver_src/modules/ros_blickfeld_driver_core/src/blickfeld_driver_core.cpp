...
void BlickfeldDriverCore::init(const std::string& host, const PointCloudOptions& point_cloud_options,
  ...
  imu_parser_ = BlickfeldDriverImuParser(imu_options.lidar_frame_id);
  ...
  while (isRunning() == true) {
    if (isImuStreaming() == false) {
      ...
      try {
        ...
        for (std::pair<SensorMsgImuPtr, time_t> imu_message : imu_messages) {
          publishImu(imu_message.first, imu_message.second);
        }
      } catch (std::exception const& e) {
